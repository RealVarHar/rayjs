...
function UpdateCameraPlayerBoundsPush(camera, player, envItems, envItemsLength, delta, width, height){
    ...
    let bboxWorldMin = GetScreenToWorld2D((Vector2){ (1 - bbox.x)*0.5f*width, (1 - bbox.y)*0.5f*height }, *camera);
    let bboxWorldMax = GetScreenToWorld2D((Vector2){ (1 + bbox.x)*0.5f*width, (1 + bbox.y)*0.5f*height }, *camera);
...
int main(void){
    ...
    while (!windowShouldClose()){
        ...
        updateCamera(&player, envItems, envItemsLength, deltaTime);
