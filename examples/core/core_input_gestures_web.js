...
//--------------------------------------------------------------------------------------
const screenWidth = 800;                  // Update depending on web canvas
...
let gestureLog = []; // The gesture log uses an array (as an inverted circular queue) to store the performed gestures
let gestureLogIndex = GESTURE_LOG_SIZE;         // The index for the inverted circular queue (moving from last to first direction, then looping around)
...
function Update(){
    ...
    finalVector = new Vector2( (angleLength*Math.sin(currentAngleRadians)) + protractorPosition.x, (angleLength*Math.sin(currentAngleRadians)) + protractorPosition.y ); // Calculate the final vector for display

    // Handle touch and mouse pointer points
    //--------------------------------------------------------------------------------------
    let MAX_TOUCH_COUNT = 32;
